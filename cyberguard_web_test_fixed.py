#!/usr/bin/env python3
"""
CyberGuard Web Test Interface - FIXED VERSION
Fixed regex patterns and improved error handling
"""
...
class USSDSecurityEngine:
    def __init__(self):
        ...
        try:
            ...
            # Fix suspicious patterns to be valid regex
            self._fix_suspicious_patterns()
            ...
            print(f"   - Suspicious patterns: {len(self.database['suspicious_patterns'])}")
            
        except Exception as e:
            print(f"❌ Error loading database: {e}")
            # Fallback database with fixed patterns
            self.database = {
                "safe_ussd_codes": ["*901#", "*902#", "*909#", "*123#", "*124#"],
                "scam_keywords": ["password", "bvn", "pin", "won", "prize"],
                "suspicious_patterns": [r"\*.*\*.*\*.*\*.*#", r"\*.*password.*", r"\*.*bvn.*"],
                "rules": {"safe_prefixes": ["*123", "*901"]}
            }
    
    def _fix_suspicious_patterns(self):
        """Fix suspicious patterns to be valid regex"""
        fixed_patterns = []
        for pattern in self.database.get("suspicious_patterns", []):
            # Convert simple patterns to proper regex
            if pattern == "*xxx*xxx*xxx*xxx#":
                fixed_patterns.append(r"\*.*\*.*\*.*\*.*#")
            elif pattern == "*xxx*password*":
                fixed_patterns.append(r"\*.*password.*")
            elif pattern == "*xxx*bvn*":
                fixed_patterns.append(r"\*.*bvn.*")
            elif pattern == "*xxx*pin*":
                fixed_patterns.append(r"\*.*pin.*")
            elif pattern == "*xxx*verif*":
                fixed_patterns.append(r"\*.*verif.*")
            else:
                # Escape the pattern for regex
                fixed_pattern = re.escape(pattern).replace("xxx", ".*")
                fixed_patterns.append(fixed_pattern)
        
        self.database["suspicious_patterns"] = fixed_patterns
    ...
    def check_ussd(self, code):
        ...
        try:
            ...
            print(f"❌ Error checking USSD code '{code}': {e}")
            return self._create_result(False, 0, "❌ Error checking code", "gray")
    ...
    def _has_suspicious_pattern(self, code):
        try:
            ...
            print(f"❌ Regex error with pattern: {e}")
            return False
...
class CyberGuardHandler(BaseHTTPRequestHandler):
        ...
        <head>
            ...
            <style>
                * { margin: 0; padding: 0; box-sizing: border-box; }
                ...
                .example.safe { border-left-color: #28a745; }
                .example.danger { border-left-color: #dc3545; }
                .example.warning { border-left-color: #ffc107; }
                .status {
                    margin-top: 20px;
                    padding: 10px;
                    border-radius: 8px;
        ...
        <body>
            <div class="container">
                ...
                <div class="status" id="status">
                    System Ready - Enter a USSD code to check security
                </div>
            ...
            <script>
                ...
                function checkCode(code) {
                    if (!code.trim()) {
                        updateStatus('Please enter a USSD code', true);
                        return;
                    }
                    
                    updateStatus('Checking security...');
                    ...
                    fetch('/check?code=' + encodeURIComponent(code))
                        ...
                        .then(data => {
                            ...
                            updateStatus(`Security check complete - Confidence: ${data.confidence}%`);
                        })
                        .catch(error => {
                            console.error('Error:', error);
                            updateStatus('Error checking code. Please try again.', true);
                        });
                }
                ...
                // Auto-check when example is clicked
    ...
    def _handle_api_check(self):
        try:
            ...
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            error_result = {"error": str(e), "message": "Internal server error"}
    ...
    def _handle_form_check(self):
        try:
            ...
            self.send_header('Content-type', 'text/html')
            self.end_headers()
            self.wfile.write(b'<h1>Error</h1><p>Something went wrong. Please try again.</p>')
    
    def _serve_stats(self):
        stats = {
            "safe_codes": len(security_engine.database["safe_ussd_codes"]),
            "scam_patterns": len(security_engine.database["scam_keywords"]),
            "suspicious_patterns": len(security_engine.database["suspicious_patterns"]),
